...
async function tryHandleAsCommand(text, onProgress) {
  const t = text.trim();
  if (!t) return null;
  // Explicit memory works in both local and API modes and never needs a
  // model call. Prefer “记住：内容” so the saved note remains unambiguous.
  const memoryMatch = t.match(/^(?:请|帮我|麻烦)?(?:记住|记下|保存)(?:这件事|一下)?[：:,，\s]*(.*)$/);
  if (memoryMatch) {
    let content = memoryMatch[1].trim();
    if (!content || content === "这件事") {
      const previous = history.slice().reverse().find((m) => m && m.role === "user" && m.content !== text);
      content = previous && previous.content ? previous.content : "";
    }
    if (!content) return { ok: false, text: "想记住什么？请在“记住：”后写下内容。" };
    const result = window.minicpm && window.minicpm.saveMemoryNote
      ? await window.minicpm.saveMemoryNote({ content }) : { ok: false };
    return result && result.ok ? { ok: true, text: "我记住了。" } : { ok: false, text: "保存笔记失败。" };
  }
  // Apple Music is a fixed, main-process allowlist. It works in both local
  // and API modes and never gives chat text shell execution privileges.
  if (looksLikeAppleMusicCommand(t) && window.minicpm && window.minicpm.musicControl) {
    return await window.minicpm.musicControl({ text: t });
  }
  // API mode deliberately has no local model/update/LoRA command surface.
...
function looksLikeAppleMusicCommand(text) {
  return /^(?:请|帮我|麻烦)?(?:打开)?(?:Apple\s*Music|音乐)?(?:里|中)?(?:播放|暂停|停止|继续|恢复|下一首|下一曲|上一首|上一曲|切歌|现在|当前|把(?:音乐)?音量|(?:音乐)?音量)/i.test(text);
}
...
async function submit(text) {
  ...
  try {
    ...
    if (cmd) {
      ...
      if (cmd.resetHistory) {
        ...
        saveChatHistory("assistant", cmd.text);
      }
  ...
  let timeContext = "";
  try {
    timeContext = (window.minicpm && typeof window.minicpm.getChatContext === "function")
      ? await window.minicpm.getChatContext()
      : "";
  } catch {}
  ...
  try {
      ...
      const stream = new ReadableStream({
        start(controller) {
          window.minicpm.remoteChat({
            ...
            controller.enqueue(encoder.encode(`data: ${JSON.stringify(frame)}\n\n`));
          }).then((handle) => { remoteCancel = handle.cancel; }).catch((err) => {
            controller.enqueue(encoder.encode(`data: ${JSON.stringify({ event: "error", message: err.message || String(err) })}\n\n`));
            controller.close();
          });
        },
        cancel() { if (remoteCancel) { remoteCancel(); remoteCancel = null; } },
      });
      abortCtrl.signal.addEventListener("abort", () => { if (remoteCancel) { remoteCancel(); remoteCancel = null; } }, { once: true });
      resp = new Response(stream, { status: 200 });
    } else resp = await fetch(sidecarUrl + "/api/chat", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({
        messages: messagesToSend,
    ...
    saveChatHistory("assistant", replyAcc);
    if (speakEl) {
