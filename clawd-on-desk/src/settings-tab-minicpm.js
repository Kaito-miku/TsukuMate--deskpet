...
(function initSettingsTabMinicpm(root) {
  ...
  async function probeHealth(ctx) {
    ...
    const llamaReady = !!(st && st.remote) || (sidecarReady
  ...
  async function renderBehaviorSection(box, ctx) {
    ...
    // Thinking is a llama.cpp-specific feature, so don't present a switch
    // that a generic OpenAI-compatible endpoint cannot reliably honor.
    if (!(st && st.remote)) {
      rows.appendChild(switchRow(
    ...
    // Apple Music search scope is independent from inference mode: remote API
    // chat users should have the same local media controls as local users.
    const musicSection = helpers.buildSection("Apple Music", []);
    const musicRows = musicSection.querySelector(".section-rows");
    let musicConfig = { playlist: "" };
    try { musicConfig = await window.minicpmSettings.getAppleMusicConfig() || musicConfig; } catch {}
    const playlist = el("input", {
      type: "text",
      className: "minicpm-adapter-editor-input",
      placeholder: "留空则搜索整个音乐资料库",
      maxlength: "160",
    });
    playlist.value = musicConfig.playlist || "";
    const savePlaylist = async () => {
      const result = await window.minicpmSettings.setAppleMusicConfig({ playlist: playlist.value });
      if (ops && ops.showToast) {
        ops.showToast(result && result.ok ? "Apple Music 找歌位置已保存。" : `保存失败：${(result && result.error) || ""}`, { error: !(result && result.ok) });
      }
    };
    musicRows.appendChild(el("div", { className: "row minicpm-api-field" },
      el("div", { className: "row-text" },
        el("span", { className: "row-label" }, "默认找歌位置"),
        el("span", { className: "row-desc" }, "填写 Apple Music 播放列表名称；留空会搜索整个资料库。"),
        playlist
      )
    ));
    musicRows.appendChild(el("div", { className: "row" }, el("div", { className: "row-text" }),
      el("div", { className: "row-control" }, softBtn("保存", () => { void savePlaylist(); }, { accent: true }))));
    box.appendChild(musicSection);
  }

  async function renderInferenceSection(box, ctx) {
    box.innerHTML = "";
    let config = { inference_mode: "local", api_endpoint: "", api_model: "", api_persona: "", api_key_configured: false };
    try { config = await window.minicpmSettings.getInferenceConfig() || config; } catch {}
    const apiMode = config.inference_mode === "api";
    box.appendChild(sectionTitle(t("minicpmSectionInference")));
    const section = helpers.buildSection("", []);
    const rows = section.querySelector(".section-rows");
    const modeRow = el("div", { className: "row" });
    modeRow.appendChild(el("div", { className: "row-text" }, el("span", { className: "row-label" }, t("minicpmSectionInference"))));
    const segmented = el("div", { className: "segmented" });
    for (const [value, label] of [["local", t("minicpmInferenceLocal")], ["api", t("minicpmInferenceApi")]]) {
      const button = el("button", { type: "button", className: config.inference_mode === value ? "active" : "" }, label);
      button.addEventListener("click", async () => {
        if (value === config.inference_mode) return;
        if (value === "api" && (!config.api_endpoint || !config.api_model || !config.api_key_configured)) {
          if (ops && ops.showToast) ops.showToast(t("minicpmApiTestFailed") + "configure the API first", { error: true });
          return;
        }
        const r = await window.minicpmSettings.setInferenceConfig({ ...config, inference_mode: value });
        if (!r || !r.ok) { if (ops && ops.showToast) ops.showToast(t("minicpmApiTestFailed") + ((r && r.error) || ""), { error: true }); return; }
        void ctx.refreshAll();
      });
      segmented.appendChild(button);
    }
    modeRow.appendChild(el("div", { className: "row-control" }, segmented));
    rows.appendChild(modeRow);
    {
      const endpoint = el("input", { type: "url", className: "minicpm-adapter-editor-input", placeholder: "https://api.example.com/v1/chat/completions" });
      endpoint.value = config.api_endpoint || "";
      const model = el("input", { type: "text", className: "minicpm-adapter-editor-input", placeholder: "gpt-4o-mini" });
      model.value = config.api_model || "";
      ...
      const key = el("input", { type: "password", className: "minicpm-adapter-editor-input", placeholder: config.api_key_configured ? t("minicpmApiKeyConfigured") : t("minicpmApiKey") });
      const field = (label, input) => el("div", { className: "row minicpm-api-field" }, el("div", { className: "row-text" }, el("span", { className: "row-label" }, label), input));
      rows.appendChild(field(t("minicpmApiEndpoint"), endpoint));
      rows.appendChild(field(t("minicpmApiModel"), model));
      rows.appendChild(el("div", { className: "row" }, el("div", { className: "row-text" }, el("span", { className: "row-label" }, t("minicpmDiaryEnabled"))), el("div", { className: "row-control" }, diaryEnabled)));
      rows.appendChild(field(t("minicpmDiaryTime"), diaryTime));
      rows.appendChild(field(t("minicpmApiKey"), key));
      const actions = el("div", { className: "row-control" });
      const save = async (test) => {
        const r = await window.minicpmSettings.setInferenceConfig({ inference_mode: "api", api_endpoint: endpoint.value, api_model: model.value, diary_enabled: diaryEnabled.checked, diary_time: diaryTime.value, api_key: key.value });
        if (!r || !r.ok) { if (ops && ops.showToast) ops.showToast(t("minicpmApiTestFailed") + ((r && r.error) || ""), { error: true }); return; }
        if (test) {
          const checked = await window.minicpmSettings.testApi();
          if (ops && ops.showToast) ops.showToast(checked && checked.ok ? t("minicpmApiTestSuccess") : t("minicpmApiTestFailed") + ((checked && checked.error) || ""), { error: !(checked && checked.ok) });
        }
        void ctx.refreshAll();
      };
      actions.appendChild(softBtn(t("minicpmApiSave"), () => { void save(false); }));
      actions.appendChild(softBtn(t("minicpmApiSaveAndTest"), () => { void save(true); }, { accent: true }));
      if (config.api_key_configured) actions.appendChild(softBtn(t("minicpmApiClearKey"), async () => { await window.minicpmSettings.clearApiKey(); void ctx.refreshAll(); }));
      rows.appendChild(el("div", { className: "row" }, el("div", { className: "row-text" }), actions));
      const historyButton = softBtn(t("minicpmChatHistoryOpen"), async () => {
        const result = await window.minicpmSettings.openChatHistoryDir();
        if (result && !result.ok) alert(result.error || t("minicpmChatHistoryOpenFailed"));
      });
      rows.appendChild(el("div", { className: "row" }, el("div", { className: "row-text" }, el("span", { className: "row-label" }, t("minicpmChatHistoryOpen"))), el("div", { className: "row-control" }, historyButton)));
      const diaryActions = el("div", { className: "row-control" });
      diaryActions.appendChild(softBtn(t("minicpmDiaryOpen"), () => { void window.minicpmSettings.openDiaryDir(); }));
      diaryActions.appendChild(softBtn(t("minicpmDiaryGenerate"), async () => {
        const result = await window.minicpmSettings.generateDiary();
        if (result && !result.ok && ops && ops.showToast) ops.showToast(result.error || "Diary generation failed", { error: true });
      }));
      rows.appendChild(el("div", { className: "row" }, el("div", { className: "row-text" }, el("span", { className: "row-label" }, t("minicpmDiaryEnabled"))), diaryActions));
    }
    box.appendChild(section);
  }
  ...
  async function refreshAll(ctx) {
    ...
    ctx.adapterBox.style.display = apiMode ? "none" : "";
    ctx.advancedBox.style.display = apiMode ? "none" : "";
    if (apiMode) return;
  ...
  async function render(parent) {
    ...
    const ctx = {
      ...
      inferenceBox: el("div", { className: "minicpm-section-box" }),
