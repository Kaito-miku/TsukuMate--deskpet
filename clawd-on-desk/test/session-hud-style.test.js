...
describe("session HUD visual shell", () => {
  ...
  it("reserves row-level space for the task-card controls", () => {
    assert.match(sessionHudHtml, /\.hud\.has-pin\s+\.row\s*\{[\s\S]*padding-right:\s*48px;[\s\S]*\}/);
    assert.doesNotMatch(sessionHudHtml, /\.hud\.has-pin\s+\.row\s+\.right\s*\{[\s\S]*padding-right:/);
    assert.match(sessionHudHtml, /\.collapse-btn\s*\{/);
    assert.match(sessionHudRenderer, /function createCollapseButton\(\)/);
    assert.match(sessionHudRenderer, /function createTaskCard\(session\)/);
  });
