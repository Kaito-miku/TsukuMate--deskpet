...
describe("session HUD layout", () => {
  it("uses only the most recent Codex session for the task card", () => {
    const snapshot = {
      sessions: [mkSession("new"), mkSession("old"), { ...mkSession("other"), agentId: "claude-code" }],
      orderedIds: ["new", "other", "old"],
    };
    assert.deepStrictEqual(computeTaskCardLayout(snapshot), {
