...
const bubbleHtml = fs.readFileSync(path.join(__dirname, "..", "src", "renderer/permission-bubble/bubble.html"), "utf8");
...
describe("permission bubble HTML assets", () => {
  it("loads external bubble CSS and renderer scripts under CSP", () => {
    ...
    assert.ok(bubbleHtml.includes('<script src="../../shared/ui/bubble-format.js"></script>'));
    ...
    assert.ok(bubbleHtml.indexOf('<script src="../../shared/ui/bubble-format.js"></script>') < bubbleHtml.indexOf('<script src="bubble-renderer.js"></script>'));
  });
});
