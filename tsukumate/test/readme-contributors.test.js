...
test("README files keep TsukuMate as the product identity", () => {
  for (const filename of ALL_READMES) {
    const markdown = readReadme(filename);
    assert.match(markdown, /<h1 align="center">TsukuMate<\/h1>/, `${filename} should use the MiniCPM product title`);
    ...
    assert.ok(markdown.includes("Kaito-miku/TsukuMate--deskpet"), `${filename} should link to the OpenBMB repository or releases`);
  }
});
