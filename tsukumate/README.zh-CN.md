...
TsukuMate 是一个由 MiniCPM 驱动的本地优先 AI 桌宠。它把桌面宠物、本地 MiniCPM 聊天气泡、首次启动模型引导，以及 AI 编程 Agent 状态反馈整合到同一个应用里。
...
从 [TsukuMate Releases](https://github.com/Kaito-miku/TsukuMate--deskpet/releases) 下载预构建安装包。
...
应用目录为 `tsukumate`，用于兼容上游 Electron 结构和现有 hook 路径。
...
TsukuMate 使用 OpenBMB MiniCPM 模型资源，并在 [NOTICE.md](NOTICE.md) 中保留上游桌宠 UI 基础的归属说明。模型权重与第三方资源遵循各自许可证。
