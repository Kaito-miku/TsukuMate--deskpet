...
const CLAWD_SERVER_ID = "tsukumate";
const LEGACY_SERVER_IDS = new Set([CLAWD_SERVER_ID, "clawd-on-desk"]);
const CLAWD_SERVER_HEADER = "x-tsukumate-server";
const LEGACY_SERVER_HEADER = "x-clawd-server";
...
const RUNTIME_CONFIG_PATH = path.join(os.homedir(), ".tsukumate", "runtime.json");
const LEGACY_RUNTIME_CONFIG_PATH = path.join(os.homedir(), ".clawd", "runtime.json");
...
function readRuntimeConfig() {
  try {
    const runtimePath = fs.existsSync(RUNTIME_CONFIG_PATH) ? RUNTIME_CONFIG_PATH : LEGACY_RUNTIME_CONFIG_PATH;
...
function isClawdResponse(res, body) {
  if (LEGACY_SERVER_IDS.has(readHeader(res, CLAWD_SERVER_HEADER)) || LEGACY_SERVER_IDS.has(readHeader(res, LEGACY_SERVER_HEADER))) return true;
  if (!body) return false;
  try {
    const data = JSON.parse(body);
    return data && LEGACY_SERVER_IDS.has(data.app);
  } catch {
    return false;
  }
}
...
function postStateToPort(port, payload, timeoutMs, callback, options = {}) {
  ...
  const req = httpRequest(
    ...
    (res) => {
      if (LEGACY_SERVER_IDS.has(readHeader(res, CLAWD_SERVER_HEADER)) || LEGACY_SERVER_IDS.has(readHeader(res, LEGACY_SERVER_HEADER))) {
        res.resume();
        callback(true, port);
        return;
      }
...
function postPermissionToPort(port, payload, timeoutMs, callback, options = {}) {
  ...
  const req = httpRequest(
    ...
    (res) => {
      ...
      res.on("end", () => {
        finish(LEGACY_SERVER_IDS.has(readHeader(res, CLAWD_SERVER_HEADER)) || LEGACY_SERVER_IDS.has(readHeader(res, LEGACY_SERVER_HEADER)), responseBody, res.statusCode || 0);
      });
    }
  );
