...
function extractCopyrightShort(copyright) {
  if (!copyright) return "\u00a9 2026 Kaito-miku and contributors";
  const match = String(copyright).match(/Copyright\s*\u00a9?\s*(\d{4})\s+([^.\n]+)/i);
  if (match) return `\u00a9 ${match[1]} ${match[2].trim()}`;
  return "\u00a9 2026 Kaito-miku and contributors";
}
...
const productName = (pkg.build && pkg.build.productName) || "TsukuMate";
