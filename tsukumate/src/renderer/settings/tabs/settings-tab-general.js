  ...
  function buildVolumeSliderRow() {
    ...
    row.innerHTML =
      ...
      `<div class="row-control volume-control">` +
        ...
        `<label class="macos-number-field"><input type="number" class="volume-readout" min="0" max="100" step="1" aria-label="${t("rowVolume")}" /><span>%</span></label>` +
      `</div>`;
    ...
    function commitTypedVolume() {
      const raw = Number(readout.value);
      if (!Number.isFinite(raw)) return applySliderValue(getSnapshotVolumePct());
      const pct = Math.max(0, Math.min(100, Math.round(raw)));
      applySliderValue(pct);
      slider.dispatchEvent(new Event("change"));
    }
    readout.addEventListener("keydown", (event) => {
      if (event.key === "Escape") { applySliderValue(getSnapshotVolumePct()); readout.blur(); }
      if (event.key === "Enter") { event.preventDefault(); commitTypedVolume(); readout.blur(); }
    });
    readout.addEventListener("blur", commitTypedVolume);
  ...
  function buildTextScaleRow() {
    ...
    row.innerHTML =
      ...
      `<div class="row-control volume-control text-scale-control">` +
        ...
        `<label class="macos-number-field"><input type="number" class="volume-readout text-scale-readout" min="${TEXT_SCALE_UI_MIN}" max="${TEXT_SCALE_UI_MAX}" step="${TEXT_SCALE_UI_STEP}" /><span>%</span></label>` +
      `</div>`;
    ...
    function commitTypedTextScale() {
      if (readout.value === "") { syncFromContext(); return; }
      const pct = snapTextScalePct(readout.value);
      paint(pct);
      commit(pct);
    }
    readout.addEventListener("keydown", (event) => {
      if (event.key === "Escape") { syncFromContext(); readout.blur(); }
      if (event.key === "Enter") { event.preventDefault(); commitTypedTextScale(); readout.blur(); }
    });
    readout.addEventListener("blur", commitTypedTextScale);
  ...
  function buildSizeSliderRow() {
    ...
    row.innerHTML =
      ...
      `<div class="row-control volume-control size-control">` +
        ...
        `<label class="macos-number-field"><input type="number" class="volume-readout text-scale-readout size-readout" min="${helpers.SIZE_UI_MIN}" max="${helpers.SIZE_UI_MAX}" step="1" /><span>%</span></label>` +
      `</div>`;
    ...
    function commitTypedSize() {
      const raw = Number(readout.value);
      if (!Number.isFinite(raw)) return applyLocalValue(readers.readSizeUiFromSnapshot());
      const next = Math.max(helpers.SIZE_UI_MIN, Math.min(helpers.SIZE_UI_MAX, Math.round(raw)));
      applyLocalValue(next);
      void controller.change(next);
    }
    readout.addEventListener("keydown", (event) => {
      if (event.key === "Escape") { controller.syncFromSnapshot(); readout.blur(); }
      if (event.key === "Enter") { event.preventDefault(); commitTypedSize(); readout.blur(); }
    });
    readout.addEventListener("blur", commitTypedSize);
