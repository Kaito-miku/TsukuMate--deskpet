...
async function openImageViewer(attachment) {
  ...
  const image = new Image(); image.onload = () => { previewImage = image; previewAttachmentId = attachment.id; $("image-viewer-title").textContent = result.image.name || "图片预览"; $("image-ocr-result").hidden = true; $("image-ocr-text").value = ""; $("image-more-menu").hidden = true; $("image-markup-panel").hidden = true; $("image-markup-toggle").setAttribute("aria-pressed", "false"); selectPreviewTool(null); drawPreviewBase(); $("image-viewer").hidden = false; }; image.onerror = () => { $("send-status").textContent = "图片无法显示"; }; image.src = result.image.dataUrl;
}
...
$("image-markup-toggle").onclick = () => { const panel = $("image-markup-panel"); panel.hidden = !panel.hidden; $("image-markup-toggle").setAttribute("aria-pressed", String(!panel.hidden)); $("image-more-menu").hidden = true; };
