...
function fitPreviewImage() { const canvas = $("image-canvas"); const wrap = document.querySelector(".image-canvas-wrap"); if (!canvas.width || !wrap) return; previewFitZoom = Math.min(1, Math.max(.08, (wrap.clientWidth - 36) / canvas.width, (wrap.clientHeight - 36) / canvas.height)); previewZoom = previewFitZoom; applyPreviewZoom(); }
...
function restoreAnnotationHistory(index) { annotationHistoryIndex = Math.max(0, Math.min(annotationHistory.length - 1, index)); annotationOps = structuredClone(annotationHistory[annotationHistoryIndex]); selectedAnnotationIndexes.clear(); annotationSelection = null; renderAnnotations(); updateAnnotationHistoryButtons(); }
function clearAnnotations() { if (!annotationOps.length) return; annotationOps = []; selectedAnnotationIndexes.clear(); annotationSelection = null; commitAnnotations(); renderAnnotations(); }
...
async function openImageViewer(attachment) {
  ...
  const image = new Image(); image.onload = () => { previewImage = image; previewAttachmentId = attachment.id; $("image-viewer-title").textContent = result.image.name || "图片预览"; $("image-ocr-result").hidden = true; $("image-ocr-text").value = ""; $("image-more-menu").hidden = true; $("image-markup-panel").hidden = true; $("image-markup-toggle").setAttribute("aria-pressed", "false"); selectPreviewTool("select"); drawPreviewBase(); updateAnnotationHistoryButtons(); $("image-viewer").hidden = false; requestAnimationFrame(fitPreviewImage); }; image.onerror = () => { $("send-status").textContent = "图片无法显示"; }; image.src = result.image.dataUrl;
}
...
document.addEventListener("pointerdown", (event) => { if (!event.target.closest("#attachment-context-menu")) closeAttachmentContextMenu(); if (!event.target.closest("#image-more, #image-more-menu")) { $("image-more-menu").hidden = true; $("image-more").setAttribute("aria-expanded", "false"); } });
...
$("image-canvas").addEventListener("pointerdown", (event) => { const tool = previewTool || "select"; const canvas = event.currentTarget; const point = previewCanvasPoint(event); const width = Math.max(2, Number($("image-brush-size").value) * (canvas.width / 900)); if (tool === "text") { const text = prompt("输入批注文字"); if (text) { annotationOps.push({ type: "text", text: text.slice(0, 300), x: point.x, y: point.y, color: $("image-color").value, size: Math.max(16, Math.round(width * 5)) }); commitAnnotations(); renderAnnotations(); } return; } previewDrawing = true; previewLastPoint = point; canvas.setPointerCapture(event.pointerId); if (tool === "select") { annotationSelection = { x: point.x, y: point.y, w: 0, h: 0 }; selectedAnnotationIndexes.clear(); } else if (tool === "brush") annotationDraft = { type: "brush", points: [point], color: $("image-color").value, width }; else if (["line", "rect", "circle"].includes(tool)) annotationDraft = { type: tool, x1: point.x, y1: point.y, x2: point.x, y2: point.y, color: $("image-color").value, width }; else if (tool === "eraser") { const radius = Math.max(width * 3, 16); annotationOps = annotationOps.filter((op) => { const box = opBounds(op); const dx = Math.max(box.x - point.x, 0, point.x - (box.x + box.w)); const dy = Math.max(box.y - point.y, 0, point.y - (box.y + box.h)); return Math.hypot(dx, dy) > radius; }); commitAnnotations(); renderAnnotations(); } });
$("image-canvas").addEventListener("pointermove", (event) => { if (!previewDrawing) return; const point = previewCanvasPoint(event); if (previewTool === "select" && annotationSelection) { annotationSelection.w = point.x - annotationSelection.x; annotationSelection.h = point.y - annotationSelection.y; renderAnnotations(); return; } if (previewTool === "brush" && annotationDraft) annotationDraft.points.push(point); else if (["line", "rect", "circle"].includes(previewTool) && annotationDraft) { annotationDraft.x2 = point.x; annotationDraft.y2 = point.y; } else if (previewTool === "eraser") { const radius = Math.max(Number($("image-brush-size").value) * 3, 16); annotationOps = annotationOps.filter((op) => { const box = opBounds(op); const dx = Math.max(box.x - point.x, 0, point.x - (box.x + box.w)); const dy = Math.max(box.y - point.y, 0, point.y - (box.y + box.h)); return Math.hypot(dx, dy) > radius; }); } renderAnnotations(); });
$("image-canvas").addEventListener("pointerup", (event) => { if (!previewDrawing) return; previewDrawing = false; if (previewTool === "select" && annotationSelection) { const raw = annotationSelection; const area = { x: Math.min(raw.x, raw.x + raw.w), y: Math.min(raw.y, raw.y + raw.h), w: Math.abs(raw.w), h: Math.abs(raw.h) }; selectedAnnotationIndexes = new Set(annotationOps.map((op, index) => rectsIntersect(area, opBounds(op)) ? index : -1).filter((index) => index >= 0)); annotationSelection = null; } else if (annotationDraft) { annotationOps.push(annotationDraft); annotationDraft = null; commitAnnotations(); } else if (previewTool === "eraser") commitAnnotations(); previewLastPoint = null; renderAnnotations(); event.currentTarget.releasePointerCapture?.(event.pointerId); });
document.addEventListener("keydown", (event) => { if ($("image-viewer").hidden || !selectedAnnotationIndexes.size || !["Backspace", "Delete"].includes(event.key)) return; event.preventDefault(); annotationOps = annotationOps.filter((_op, index) => !selectedAnnotationIndexes.has(index)); selectedAnnotationIndexes.clear(); commitAnnotations(); renderAnnotations(); });
