...
function selectPreviewTool(tool) { previewTool = tool; $("image-select").classList.toggle("active", !previewTool); $("image-brush").classList.toggle("active", previewTool === "brush"); $("image-text").classList.toggle("active", previewTool === "text"); $("image-brush").setAttribute("aria-pressed", String(previewTool === "brush")); $("image-text").setAttribute("aria-pressed", String(previewTool === "text")); $("image-canvas").style.cursor = previewTool ? "crosshair" : "default"; }
async function openImageViewer(attachment) {
  ...
  const image = new Image(); image.onload = () => { previewImage = image; previewAttachmentId = attachment.id; $("image-viewer-title").textContent = result.image.name || "图片预览"; $("image-ocr-result").hidden = true; $("image-ocr-text").value = ""; $("image-more-menu").hidden = true; $("image-markup-panel").hidden = true; $("image-markup-toggle").setAttribute("aria-pressed", "false"); selectPreviewTool(null); drawPreviewBase(); $("image-viewer").hidden = false; requestAnimationFrame(fitPreviewImage); }; image.onerror = () => { $("send-status").textContent = "图片无法显示"; }; image.src = result.image.dataUrl;
}
function closeImageViewer() { $("image-viewer").hidden = true; $("image-more-menu").hidden = true; $("image-markup-panel").hidden = true; previewImage = null; previewAttachmentId = null; previewDrawing = false; selectPreviewTool(null); }
...
$("image-copy").onclick = copyPreviewImage; $("image-download").onclick = downloadPreviewImage; $("image-select").onclick = () => selectPreviewTool(null); $("image-brush").onclick = () => selectPreviewTool("brush"); $("image-text").onclick = () => selectPreviewTool("text"); $("image-clear").onclick = drawPreviewBase; $("image-panel-reset").onclick = drawPreviewBase;
...
$("image-canvas").addEventListener("pointerup", (event) => { previewDrawing = false; previewLastPoint = null; event.currentTarget.releasePointerCapture?.(event.pointerId); });
