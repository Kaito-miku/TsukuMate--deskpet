...
$("image-canvas").addEventListener("pointerdown", (event) => { const tool = previewTool || "select"; const canvas = event.currentTarget; const point = previewCanvasPoint(event); const width = Math.max(2, Number($("image-brush-size").value) * (canvas.width / 900)); if (tool === "text") { const text = prompt("输入批注文字"); if (text) { annotationOps.push({ type: "text", text: text.slice(0, 300), x: point.x, y: point.y, color: $("image-color").value, size: Math.max(16, Math.round(width * 5)) }); commitAnnotations(); renderAnnotations(); } return; } previewDrawing = true; previewLastPoint = point; canvas.setPointerCapture(event.pointerId); if (tool === "select") { const bounds = selectedAnnotationBounds(); if (bounds && point.x >= bounds.x && point.x <= bounds.x + bounds.w && point.y >= bounds.y && point.y <= bounds.y + bounds.h) { annotationMoveOrigin = point; annotationMoveSnapshot = structuredClone(annotationOps); } else { annotationSelection = { x: point.x, y: point.y, w: 0, h: 0 }; selectedAnnotationIndexes.clear(); hideAnnotationSelectionMenu(); } } else if (tool === "brush") annotationDraft = { type: "brush", points: [point], color: $("image-color").value, width }; else if (["line", "rect", "circle"].includes(tool)) annotationDraft = { type: tool, x1: point.x, y1: point.y, x2: point.x, y2: point.y, color: $("image-color").value, width }; else if (tool === "eraser") { const radius = Math.max(width * 3, 16); annotationOps = annotationOps.filter((op) => { const box = opBounds(op); const dx = Math.max(box.x - point.x, 0, point.x - (box.x + box.w)); const dy = Math.max(box.y - point.y, 0, point.y - (box.y + box.h)); return Math.hypot(dx, dy) > radius; }); renderAnnotations(); } });
$("image-canvas").addEventListener("pointermove", (event) => { if (!previewDrawing) return; const point = previewCanvasPoint(event); if (previewTool === "select" && annotationMoveOrigin && annotationMoveSnapshot) { annotationOps = structuredClone(annotationMoveSnapshot); const dx = point.x - annotationMoveOrigin.x; const dy = point.y - annotationMoveOrigin.y; for (const index of selectedAnnotationIndexes) moveAnnotation(annotationOps[index], dx, dy); renderAnnotations(); showAnnotationSelectionMenu(); return; } if (previewTool === "select" && annotationSelection) { annotationSelection.w = point.x - annotationSelection.x; annotationSelection.h = point.y - annotationSelection.y; renderAnnotations(); return; } if (previewTool === "brush" && annotationDraft) annotationDraft.points.push(point); else if (["line", "rect", "circle"].includes(previewTool) && annotationDraft) { annotationDraft.x2 = point.x; annotationDraft.y2 = point.y; } else if (previewTool === "eraser") { const radius = Math.max(Number($("image-brush-size").value) * 3, 16); annotationOps = annotationOps.filter((op) => { const box = opBounds(op); const dx = Math.max(box.x - point.x, 0, point.x - (box.x + box.w)); const dy = Math.max(box.y - point.y, 0, point.y - (box.y + box.h)); return Math.hypot(dx, dy) > radius; }); } renderAnnotations(); });
