...
function isHudSession(session) {
  return !!session && session.agentId === "codex" && !session.headless
    && session.state !== "sleeping" && !session.hiddenFromHud;
}
...
const CHEVRON_DOWN_SVG = `<svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.4" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><path d="m6 9 6 6 6-6"/></svg>`;
const CHEVRON_UP_SVG = `<svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.4" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><path d="m18 15-6-6-6 6"/></svg>`;
...
function createRowForSession(session, now) {
  ...
  progress.textContent = progressFor(session);
...
function createCollapseButton() {
  ...
  btn.innerHTML = compact ? CHEVRON_DOWN_SVG : CHEVRON_UP_SVG;
  btn.title = compact ? "展开任务卡" : "折叠任务卡";
...
function createTaskCard(session) {
  const row = document.createElement("div");
  row.className = "row task-card-row";
  const canFocus = session.canFocus === true;
  if (!canFocus) {
    row.classList.add("row-unfocusable");
    row.title = focusUnavailableTooltip(session);
...
function render() {
  ...
  hudEl.className = compact ? "hud has-pin compact" : "hud has-pin";
  if (!sessions.length) return;
  hudEl.appendChild(createTaskCard(sessions[0]));
  hudEl.appendChild(createCollapseButton());
