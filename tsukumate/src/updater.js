...
const GITHUB_RELEASES_API_PATH = productMetadata.githubReleasesApiPath
  || "/repos/Kaito-miku/TsukuMate--deskpet/releases/latest";
const GITHUB_RELEASES_WEB_PATH = (() => {
  const repo = productMetadata.repoUrl || "https://github.com/Kaito-miku/TsukuMate--deskpet";
  try {
    const u = new URL(repo);
    return `${u.pathname.replace(/\/$/, "")}/releases/latest`;
  } catch {
    return "/Kaito-miku/TsukuMate--deskpet/releases/latest";
  }
})();
const UPDATE_USER_AGENT = productMetadata.userAgent || "TsukuMate";
...
function initUpdater(ctx, deps = {}) {
  ...
  async function showUpToDateBubble(version) {
    ...
    return showInfoBubble(
      ...
      t("updateNotAvailableMsg", "TsukuMate v{version} is the latest version.").replace("{version}", displayVersion),
      {
        version,
  ...
  async function maybePromptNativeArm64Installer(release, { manual = false, currentVersion = app.getVersion() } = {}) {
    ...
    const action = await awaitBubbleAction(showBubble({
      ...
      message: t(
        ...
        "TsukuMate v{version} has a native Windows ARM64 installer. Install it for better performance and battery life?"
      ).replace("{version}", displayVersion),
      version,
      actions: [
  ...
  async function runGitUpdate(repoRoot, branch, localHead) {
    ...
    await showSuccessBubble({
      title: t("updateReady", "Update Ready"),
      message: t("gitUpdateRestarting", "Update complete. Restarting TsukuMate now..."),
    });
