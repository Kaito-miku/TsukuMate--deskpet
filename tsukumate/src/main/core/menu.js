...
const { createTranslator } = require("../../shared/i18n/i18n");
...
module.exports = function initMenu(ctx) {
  ...
  function createTray() {
    ...
    let icon;
    if (isMac) {
      icon = nativeImage.createFromPath(path.join(ASSETS_ROOT, "tray-iconTemplate.png"));
      icon.setTemplateImage(true);
    } else {
      icon = nativeImage.createFromPath(path.join(ASSETS_ROOT, "tray-icon.png")).resize({ width: 32, height: 32 });
    }
    ctx.tray = new Tray(icon);
    ctx.tray.setToolTip("TsukuMate");
    buildTrayMenu();
  }
