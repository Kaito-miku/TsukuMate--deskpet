...
const LEGACY_PROTOCOL_SCHEME = "clawd";
...
function extractClawdProtocolUrls(argv) {
  ...
  return argv.filter((arg) => typeof arg === "string" && [CLAWD_PROTOCOL_SCHEME, LEGACY_PROTOCOL_SCHEME]
    .some((scheme) => arg.toLowerCase().startsWith(`${scheme}:`)));
}
...
function createCodexPetMain(options = {}) {
  ...
  function registerProtocolClient() {
    try {
      if (app.isPackaged) {
        const primary = app.setAsDefaultProtocolClient(CLAWD_PROTOCOL_SCHEME);
        app.setAsDefaultProtocolClient(LEGACY_PROTOCOL_SCHEME);
        return primary;
      }
      if (process.argv.includes(REGISTER_PROTOCOL_DEV_ARG) || process.env.CLAWD_REGISTER_PROTOCOL_DEV === "1") {
        const appRoot = APP_ROOT;
        const primary = app.setAsDefaultProtocolClient(CLAWD_PROTOCOL_SCHEME, process.execPath, [appRoot]);
        app.setAsDefaultProtocolClient(LEGACY_PROTOCOL_SCHEME, process.execPath, [appRoot]);
        return primary;
      }
    } catch (err) {
      console.warn("TsukuMate: failed to register import protocol:", err && err.message);
    }
    return false;
  }
