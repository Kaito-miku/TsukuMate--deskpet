...
const { detectAgentInstallations: defaultDetectAgentInstallations } = require("../integrations/agents/agent-installation-detector");
...
function registerSettingsIpc(options = {}) {
  ...
  const aboutHeroSvgPath = options.aboutHeroSvgPath
    || path.join(ASSETS_ROOT, "svg", "minicpm-logo.svg");
  const disposers = [];
  ...
  handle("settings:import-user-theme-zip", async (event) => {
    ...
    try {
      result = await dialog.showOpenDialog(getDialogParent(event), {
        properties: ["openFile"],
        filters: [{ name: "TsukuMate theme zip", extensions: ["zip"] }],
      });
    } catch (err) {
