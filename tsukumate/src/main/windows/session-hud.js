...
const { PRELOAD_ROOT, RENDERER_ROOT } = require("../paths");
...
function isHudSession(session) {
  return !!session && session.agentId === "codex" && !session.headless && session.state !== "sleeping" && !session.hiddenFromHud;
}
...
// The task card represents the most recently updated Codex task. The
// full layout helper remains available for Dashboard-compatible consumers.
function computeTaskCardLayout(snapshot) {
  const layout = computeHudLayout(snapshot);
  return layout.expanded.length > 0
    ? { session: layout.expanded[0], rowCount: 1 }
    : { session: null, rowCount: 0 };
}
...
module.exports = function initSessionHud(ctx) {
  ...
  function shouldShow(snapshot = latestSnapshot) {
    ...
    const sessions = (snapshot.sessions || []).filter(isHudSession);
    const now = Date.now();
    if (sessions.some((s) => s.badge === "running" || s.state === "thinking" || s.state === "working" || s.state === "notification")) return true;
    if (sessions.some((s) => s.badge === "done" && now - (Number(s.updatedAt) || 0) < 8000)) return true;
    return clickRevealed;
  }

  function scheduleProgressExpiry(snapshot) {
    if (progressExpiryTimer) { clearTimeout(progressExpiryTimer); progressExpiryTimer = null; }
    const done = (snapshot && snapshot.sessions || [])
      .filter(isHudSession)
      .filter((s) => s.badge === "done")
      .map((s) => 8000 - (Date.now() - (Number(s.updatedAt) || 0)))
      .filter((remaining) => remaining > 0);
    if (!done.length) return;
    const delay = Math.min(...done);
    progressExpiryTimer = setTimeout(() => syncSessionHud(latestSnapshot, { sendSnapshot: false }), delay + 20);
  }
  ...
  function computeExpectedHudContentBounds(snapshot, scale = getTextScale()) {
    ...
    const layout = computeTaskCardLayout(snapshot);
  ...
  function syncSessionHud(snapshot = latestSnapshot || getCurrentSnapshot(), options = {}) {
    ...
    scheduleProgressExpiry(snapshot);
  ...
  function cleanup() {
    if (progressExpiryTimer) clearTimeout(progressExpiryTimer);
    stopAutoHidePoll();
    cancelHiddenDestroy();
