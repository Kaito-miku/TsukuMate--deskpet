...
const { APP_ROOT, SRC_ROOT, PRELOAD_ROOT, RENDERER_ROOT, ASSETS_ROOT, EXTENSIONS_ROOT } = require("./paths");
...
const { planXWaylandRelaunch } = require("./platform/linux-ozone");
...
} = require("./settings/settings-window-icon");
const createSettingsWindowRuntime = require("./windows/settings-window");
const {
  createSettingsSizePreviewSession,
} = require("./settings/settings-size-preview-session");
const { registerSettingsIpc } = require("./settings/settings-ipc");
const createSettingsEffectRouter = require("./settings/settings-effect-router");
const { registerSessionIpc } = require("./sessions/session-ipc");
const { registerPetInteractionIpc } = require("./core/pet-interaction-ipc");
const { createSystemWakeRecovery } = require("./platform/system-wake-recovery");
const { formatLocalTimestamp } = require("./core/log-timestamp");
const { launchClaudeSession, openTerminalAt } = require("./platform/launch-claude");
const { dialog: electronDialog } = require("electron");
const initPermission = require("./windows/permission");
const { registerPermissionIpc } = initPermission;
const { createTelegramApprovalSidecar } = require("./integrations/telegram/telegram-approval-sidecar");
const telegramApprovalSettings = require("./integrations/telegram/telegram-approval-settings");
const {
  buildTelegramApprovalStatus,
  isNativeTelegramApprovalSelected,
  buildTelegramStatusDiagnostic,
  formatTelegramStatusDiagnostic,
} = require("./integrations/telegram/telegram-approval-runtime-status");
const { createTelegramMigrationController } = require("./integrations/telegram/telegram-migration-controller");
const { createTelegramSidecarStatusBridge } = require("./integrations/telegram/telegram-sidecar-status-bridge");
const initUpdateBubble = require("./windows/update-bubble");
const { registerUpdateBubbleIpc } = initUpdateBubble;
const createSettingsAnimationOverridesMain = require("./settings/settings-animation-overrides-main");
const { registerSettingsAnimationOverridesIpc } = createSettingsAnimationOverridesMain;
const createShortcutRuntime = require("./core/shortcut-runtime");
const {
  findNearestWorkArea,
...
const createTopmostRuntime = require("./core/topmost-runtime");
const { WIN_TOPMOST_LEVEL } = createTopmostRuntime;
const createThemeFadeSequencer = require("./theme/theme-fade-sequencer");
const createThemeRuntime = require("./theme/theme-runtime");
const createAgentRuntimeMain = require("./integrations/agents/agent-runtime-main");
const createFloatingWindowRuntime = require("./core/floating-window-runtime");
const createPetWindowRuntime = require("./core/pet-window-runtime");
const { createQuickLauncher } = require("./windows/quick-launcher");
const createMacHideController = require("./platform/mac-hide");
const { createHardwareBuddyAdapter } = require("./integrations/hardware/hardware-buddy-adapter");
const { createLive2dAssetService } = require("./theme/live2d-assets");
const {
...
app.setName("TsukuMate");

// The product rename changes Electron's userData directory. Copy only durable
// user-owned data before prefs are loaded; legacy directories remain untouched
// as a rollback backup.
try {
  require("./core/brand-migration").migrateLegacyUserData({
    appDataDir: app.getPath("appData"),
    userDataDir: app.getPath("userData"),
    logger: (message) => console.warn(message),
  });
} catch (error) {
  console.warn("TsukuMate: legacy data migration failed:", error && error.message);
}
...
const { createSettingsController } = require("./settings/settings-controller");
...
if (!fs.existsSync(PREFS_PATH)) {
  const migratedPrefsPath = path.join(app.getPath("userData"), "clawd-prefs.json");
  if (fs.existsSync(migratedPrefsPath)) {
    try { fs.copyFileSync(migratedPrefsPath, PREFS_PATH, fs.constants.COPYFILE_EXCL); } catch {}
  }
}
...
function _writeSystemOpenAtLogin(enabled) {
  if (isLinux) {
    const launchScript = path.join(APP_ROOT, "launch.js");
...
// ── Theme loader ──
const themeLoader = require("./theme/theme-loader");
const createCodexPetMain = require("./integrations/codex-pet/codex-pet-main");
themeLoader.init(SRC_ROOT, app.getPath("userData"));
...
function flashTaskbar() {
  ...
  if (!trayFlashNormalIcon) {
    if (process.platform === "darwin") {
      trayFlashNormalIcon = nativeImage.createFromPath(
        path.join(ASSETS_ROOT, "tray-iconTemplate.png")
      );
      trayFlashNormalIcon.setTemplateImage(true);
    } else {
      trayFlashNormalIcon = nativeImage.createFromPath(
        path.join(ASSETS_ROOT, "tray-icon.png")
  ...
  if (!trayFlashHighlightIcon) {
    const flashPath = path.join(ASSETS_ROOT, "tray-icon-flash.png");
...
} = require("./integrations/agents/agent-gate");
const _permCtx = {
  get win() { return win; },
...
const _state = require("./core/state")(_stateCtx);
...
const _tick = require("./core/tick")(_tickCtx);
...
const _dashboard = require("./windows/dashboard")({
  get lang() { return lang; },
  t: (key) => translate(key),
...
_minicpmChat = require("./chat/minicpm-chat")({
  getPetWindowBounds,
  getPetHitRect: () => {
...
const _sessionHud = require("./windows/session-hud")({
  get win() { return win; },
  get petHidden() { return petWindowRuntime.isPetHidden(); },
...
const _server = require("./sessions/server")(_serverCtx);
...
if (_settingsController.get("mobilePreviewEnabled") === true) {
  const { initMobilePreviewServer } = require("./integrations/mobile/mobile-preview-server");
...
function updateLog(msg) {
  ...
  const { rotatedAppend } = require("./core/log-rotate");
...
async function initTelegramMigrationController() {
  ...
  const { envFileTokenStore } = require("./integrations/telegram/telegram-token-store");
  const {
    createClipboardFallbackDeliveryAdapter,
    createTelegramDirectSend,
    createWindowsPasteOnlyDeliveryAdapter,
  } = require("./integrations/telegram/telegram-direct-send");
  const { createTelegramNativeRunner } = require("./integrations/telegram/telegram-native-runner");
  const { createTelegramFetchTransport } = require("./integrations/telegram/telegram-fetch-transport");
  ...
  const { createTelegramCompanion } = require("./integrations/telegram/telegram-companion");
...
const _menu = require("./core/menu")(_menuCtx);
...
const _updater = require("./core/updater")(_updaterCtx);
const {
  setupAutoUpdater,
  checkForUpdates,
  getUpdateMenuItem,
...
// ── Doctor tab IPC ──
const { registerDoctorIpc } = require("./diagnostics/doctor-ipc");
registerDoctorIpc({
  ipcMain,
  app,
  shell,
...
const { createRemoteSshRuntime } = require("./integrations/remote-ssh/remote-ssh-runtime");
const { registerRemoteSshIpc } = require("./integrations/remote-ssh/remote-ssh-ipc");
...
registerSettingsIpc({
  ...
  checkForUpdates,
  aboutHeroSvgPath: path.join(ASSETS_ROOT, "svg", "minicpm-logo.svg"),
...
const _mini = require("./core/mini")(_miniCtx);
...
const _roam = require("./core/roam")(_roamCtx);
...
function installTerminalFocusExtension() {
  ...
  let extSrc = path.join(EXTENSIONS_ROOT, "vscode");
...
if (!gotTheLock) {
  ...
  app.whenReady().then(async () => {
    ...
    if (isMac && app.dock && _settingsController.get("showDock") !== false) {
      try {
        app.dock.setIcon(path.join(ASSETS_ROOT, "dock-icon.png"));
      } catch (_) {
    ...
    live2dAssets = createLive2dAssetService({
      ...
      modelRoots: [
        path.join(APP_ROOT, "live2d"),
        path.resolve(APP_ROOT, "..", "live2d"),
      ...
      coreCandidates: [
        path.join(APP_ROOT, "live2d", "live2dcubismcore.min.js"),
        path.resolve(APP_ROOT, "..", "live2d", "live2dcubismcore.min.js"),
