...
const IMPORT_MARKER_FILENAME = ".tsukumate-imported-pet.json";
...
function parseClawdImportUrl(rawUrl) {
  ...
  try {
    ...
    throw new Error("invalid TsukuMate import URL");
  }
  if (parsed.protocol !== "tsukumate:" && parsed.protocol !== "clawd:") throw new Error("unsupported protocol");

  const action = parsed.hostname || parsed.pathname.replace(/^\/+/, "");
  if (action !== "import-pet") throw new Error(`unsupported TsukuMate action: ${action || "(missing)"}`);
